...
const files = (...entries: Parameters<typeof evaluatePolicy>[1]['files']) => ({
  files: entries,
});

describe('path normalization and rules', () => {
  it('normalizes separators, dot segments, repeated separators and nested traversal', () => {
    expect(normalizeRepositoryPath('.\\src//feature/../index.ts')).toBe(
      'src/index.ts',
    );
    expect(normalizeRepositoryPath('.github/workflows/ci.yml')).toBe(
  ...
  it('denies matching add, modify, delete and both sides of rename', () => {
    ...
    for (const operation of ['add', 'modify', 'delete'] as const)
      expect(
        evaluatePolicy(policy, files({ operation, path: 'secret/key' }))
          .decision,
      ).toBe('block');
    expect(
      evaluatePolicy(
  ...
  it('enforces allow-only scope', () => {
    const policy: Policy = {
      rules: [{ id: 'scope', type: 'path-allow-only', patterns: ['src/**'] }],
    };
    expect(
      evaluatePolicy(policy, files({ operation: 'modify', path: 'src/a.ts' }))
        .decision,
    ).toBe('allow');
    expect(
      evaluatePolicy(policy, files({ operation: 'modify', path: 'README.md' }))
        .decision,
    ).toBe('block');
  });
  it('supports delete-only protection without blocking edits', () => {
    ...
    expect(
      evaluatePolicy(
        policy,
        files({ operation: 'modify', path: 'src/a.test.ts' }),
      ).decision,
    ).toBe('allow');
    expect(
      evaluatePolicy(
        policy,
        files({ operation: 'delete', path: 'src/a.test.ts' }),
      ).decision,
    ).toBe('block');
  });
});
...
describe('dependency guard', () => {
  const base = {
    production: { react: '1' },
    development: { vitest: '1', old: '1' },
  };
  const after = {
    production: { react: '2', zod: '1' },
    development: { vitest: '1', eslint: '1' },
  };
  it.each([
    'new-production',
    'new-development',
    ...
    const result = evaluatePolicy(
      { rules: [{ id: 'deps', type: 'dependency-guard', forbid: [kind] }] },
      { files: [], dependencies: { before: base, after } },
    );
  ...
  it('returns allow, warn, and block with block precedence', () => {
    ...
    expect(
      evaluatePolicy(
        {
          rules: [
            { id: 'w', type: 'path-deny', patterns: ['x'], effect: 'warn' },
          ],
        },
        files({ operation: 'add', path: 'x' }),
      ).decision,
    ).toBe('warn');
    const result = evaluatePolicy(
      {
        rules: [
          { id: 'w', type: 'path-deny', patterns: ['x'], effect: 'warn' },
          { id: 'b', type: 'path-deny', patterns: ['x'] },
        ],
      },
      files({ operation: 'add', path: 'x' }),
    );
...
describe('validation and determinism', () => {
  it.each([
    {
      rules: [
        { id: 'x', type: 'path-deny', patterns: ['a'] },
        { id: 'x', type: 'path-deny', patterns: ['b'] },
      ],
    },
